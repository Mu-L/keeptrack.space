...
- [ ] **CRITICAL — Check for lifecycle bypass**: Read the pro plugin's `addHtml()` and `addJs()` overrides. Some pro plugins call `KeepTrackPlugin.prototype.addHtml.call(this)` instead of `super.addHtml()`, **completely bypassing** the OSS parent's lifecycle methods. Any code added to the OSS parent's `addHtml()`, `addJs()`, or their registered handlers (e.g., `uiManagerFinal_`) will be **dead code** when the pro plugin is active. If the pro plugin bypasses the parent, new features (event listeners, secondary menu setup, etc.) must ALSO be added to the pro plugin's own lifecycle methods (e.g., `uiManagerFinalPro_()`).
- [ ] **`sideMenuElementName` divergence**: Pro plugins using `getSideMenuConfig()` may return a different `elementName` than the OSS parent's `sideMenuElementName` property (e.g., `satellite-lists-menu` vs `watchlist-menu`). This changes all auto-generated IDs (`*-secondary`, `*-secondary-content`, `*-secondary-btn`, `*-close-btn`). Event listeners targeting these generated IDs must use the correct prefix. Hard-coded element IDs in `sideMenuSecondaryHtml` (e.g., `id="watchlist-hide-others"`) are NOT affected and work regardless of `sideMenuElementName`.
...
11. **Pro plugin lifecycle bypass makes OSS code dead code**: Some pro plugins (e.g., `SatelliteListsPlugin`) call `KeepTrackPlugin.prototype.addHtml.call(this)` instead of `super.addHtml()`, completely skipping the OSS parent's `addHtml()`. This means event handler registrations, secondary menu listener setup, and any other code in the OSS `addHtml()` or its downstream handlers (like `uiManagerFinal_`) never executes at runtime. Symptom: changes to the OSS plugin have zero effect — no console logs, no event listeners, nothing. The pro plugin has its own parallel lifecycle methods (e.g., `uiManagerFinalPro_()` instead of `uiManagerFinal_()`). Fix: always check the pro plugin's override chain before adding code to the OSS parent. If the pro bypasses the parent, add the same setup to the pro's own lifecycle methods.

12. **Login-gated fetch with toolbar state machine**: Plugins that fetch external data should implement a toolbar state machine: (a) Guest + no data → show Fetch button, hide Refresh; (b) Guest + has data → hide Fetch, show Refresh; (c) Logged in → always hide Fetch, show Refresh, auto-fetch if empty; (d) User logs in while menu open → auto-fetch via `EventBusEvent.userLogin` handler. The `updateToolbarForLoginState_()` helper pattern centralizes this logic. Test with `vi.spyOn(PluginRegistry, 'getPlugin')` returning a mock `UserAccountPlugin` with `{ cachedUser: { id: 'test' } }` for logged-in or `{ cachedUser: null }` for guest.
